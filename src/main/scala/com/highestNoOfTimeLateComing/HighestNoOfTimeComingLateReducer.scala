...
class HighestNoOfTimeComingLateReducer extends Reducer[Text, Text, Text, Text] {
  var modifiedReduceInputs: mutable.LinkedHashMap[String, String] = _
  ...
  override def setup(context: Reducer[Text, Text, Text, Text]#Context): Unit = {
    ...
    modifiedReduceInputs = new mutable.LinkedHashMap[String, String]()
  ...
  override def reduce(
    ...
    try {
      ...
      if (!modifiedReduceInputs.contains(newKeyFields)) {
        modifiedReduceInputs.put(newKeyFields, value)
      }
    } catch {
      case nullPointerException: NullPointerException =>
        throw new Exception(
          "Unable to process the data because of null values returned"
        )
    }
