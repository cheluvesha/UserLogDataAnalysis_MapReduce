 ...
 */
class HighestNoOfIdleHoursMapper extends Mapper[Object,Text,Text,IntWritable] {
  ...
  def map(key: Object, value: Text, context: Mapper[Object, Text, Text, IntWritable]#Context): Unit = {
    ...
    val dateTime = userLogData(zero)
    val date = dateTime.substring(zero, dateTime.indexOf(' '))
